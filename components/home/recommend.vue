<template>
  <div class="recommendBox">
    <!-- 今日宝藏 -->
    <header>
      <div class="line1"></div>
      <div class="line2"></div>
      今日宝藏
    </header>
    <div class="todayBox">
      <div
        class="infoBox"
        v-for="item in todayInfo"
        :key="item.createId"
        @click="toBuy(item)"
      >
        <!-- <image :src="item.image" alt="" /> -->
        <u--image
          :showLoading="true"
          :src="item.image"
          :lazy-load="true"
          width="100%"
          radius="16rpx 16rpx 0 0"
        ></u--image>
        <h5>{{ item.name }}</h5>
        <p>{{ item.subscription }}</p>
        <footer>
          <div class="flexBox">
            <div class="left">
              <span class="font">{{ item.type }} </span>
            </div>
            <p class="right">{{ item.price }}币</p>
          </div>
        </footer>
      </div>
    </div>
    <!-- 卖家推荐 -->
    <header class="hSeller">
      <div class="line1"></div>
      <div class="line2"></div>
      卖家推荐
    </header>
    <div class="sellersBox">
      <div class="sellerBox" v-for="item in sellerInfo" :key="item.sellerId">
        <!-- <image :src="item.image" alt="" /> -->
        <u--image
          :showLoading="true"
          :src="item.image"
          :lazy-load="true"
          width="100rpx"
          height="100rpx"
        ></u--image>
        <p>{{ item.name }}</p>
        <div class="concern">
          <span>关注</span>
        </div>
      </div>
    </div>
    <!-- 臻品推荐 -->
    <header>
      <div class="line1"></div>
      <div class="line2"></div>
      臻品推荐
    </header>
    <div class="treasuresBox">
      <!-- 左边商品 -->
      <div class="leftPart" style="width: 45%">
        <div
          class="treasureBox"
          v-for="item in treasureInfo[0]"
          :key="item.createId"
          @click="toBuy(item)"
        >
          <!-- <image :src="item.image" alt="" /> -->
          <u--image
            :showLoading="true"
            :src="item.image"
            :lazy-load="true"
            width="100%"
            radius="16rpx 16rpx 0 0"
          ></u--image>
          <h5>{{ item.name }}</h5>
          <p>{{ item.subscription }}</p>
          <footer>
            <div class="tflexBox">
              <div class="tLeft">
                <span class="tfont">{{ item.type }}</span>
              </div>
              <p class="tRight">{{ item.price }}币</p>
            </div>
          </footer>
        </div>
      </div>
      <div class="rightPart" style="width: 45%">
        <div
          class="treasureBox"
          v-for="item in treasureInfo[1]"
          :key="item.createId"
          @click="toBuy(item)"
        >
          <!-- <image :src="item.image" alt="" /> -->
          <u--image
            :showLoading="true"
            :src="item.image"
            width="100%"
            radius="16rpx 16rpx 0 0"
          ></u--image>
          <h5>{{ item.name }}</h5>
          <p>{{ item.subscription }}</p>
          <footer>
            <div class="tflexBox">
              <div class="tLeft">
                <span class="tfont">{{ item.type }}</span>
              </div>
              <p class="tRight">{{ item.price }}币</p>
            </div>
          </footer>
        </div>
      </div>
    </div>
    <!-- 底部留出 -->
    <showBox></showBox>
    <!-- <footer class="showBox"></footer> -->
  </div>
</template>

<script>
import showBox from "../showBox";
import { mapState } from "vuex";
export default {
  data() {
    return {};
  },
  methods: {
    toBuy(item) {
      uni.navigateTo({
        url: "/pages/myHome/buy?info="+JSON.stringify(item),
      });
    },
  },
  computed: {
    ...mapState("recommend", ["todayInfo", "sellerInfo", "treasureInfo"]),
  },
  components: {
    showBox,
  },
};
</script>

<style lang="scss" scoped>
.recommendBox {
  header {
    position: relative;
    text-align: center;
    font-size: 28rpx;
    color: #2c2c2c;
    margin: 20rpx 0 10rpx 0;
    .line1 {
      position: absolute;
      top: 50%;
      left: 35%;
      width: 30rpx;
      border: 2rpx solid #979797;
    }
    .line2 {
      position: absolute;
      top: 50%;
      right: 35%;
      width: 30rpx;
      border: 2rpx solid #979797;
    }
  }
  .todayBox {
    display: flex;
    justify-content: space-around;
    .infoBox {
      width: 30%;
      border-radius: 32rpx 32rpx 16rpx 16rpx;
      box-shadow: 20rpx 10rpx 10rpx #cfcfcf;
      image {
        // width: 7.0625rem;
        width: 100%;
        border-radius: 32rpx 32rpx 0 0;
      }
      h5 {
        color: #3d3d3d;
        font-size: 26rpx;
        margin: 0;
      }
      p {
        color: #b1b1b1;
        font-size: 24rpx;
        margin: 10rpx 0;
      }
      footer {
        .flexBox {
          display: flex;
          justify-content: space-between;
          .left {
            position: relative;
            background-color: #ff6e00;
            border-radius: 16rpx 0 0 16rpx;
            margin-bottom: 10rpx;
            .font {
              font-size: 24rpx;
              color: #fff;
              text-align: center;
              padding: 10rpx;
            }
          }
          .right {
            color: #ff6e00;
            font-size: 32rpx;
            margin: 0 10rpx 10rpx 0;
            text-align: center;
          }
        }
      }
    }
  }
  .hSeller {
    margin-top: 30rpx; //卖家推荐顶部留出15px
  }
  .sellersBox {
    display: flex;
    justify-content: space-around;
    margin: 0 0 20rpx 0;
    .sellerBox {
      display: flex;
      flex-direction: column; //修改轴方向
      align-items: center; //上下居中
      width: 20%;
      image {
        padding-top: 10rpx;
        width: 80%;
      }
      p {
        font-size: 28rpx;
        margin: 10rpx 0;
      }
      .concern {
        width: 70%;
        text-align: center;
        border-radius: 64rpx;
        background-color: #fea766;
        span {
          font-size: 24rpx;
          color: #fff;
        }
      }
    }
  }
  .treasuresBox {
    display: flex;
    justify-content: space-around;
    // margin-left: 15px;
    // background-color: pink;
    .treasureBox {
      // background-color: #ff6e00;
      width: 100%;
      // width: 10.3125rem;
      border-radius: 32rpx 32rpx 16rpx 16rpx;
      box-shadow: 20rpx 10rpx 10rpx #cfcfcf;
      margin-bottom: 20rpx;
      // image {
      //   width: 100%;
      //   border-radius: 16px 16px 0 0;
      // }
      h5 {
        color: #3d3d3d;
        font-size: 32rpx;
        margin: 0;
      }
      p {
        color: #b1b1b1;
        font-size: 28rpx;
        margin: 10rpx 0;
      }
      footer {
        .tflexBox {
          display: flex;
          justify-content: space-between;
          .tLeft {
            position: relative;
            background-color: #ff6e00;
            border-radius: 16rpx 0 0 16rpx;
            margin-bottom: 12rpx;
            .tfont {
              font-size: 28rpx;
              color: #fff;
              text-align: center;
              padding: 10rpx;
            }
          }
          .tRight {
            color: #ff6e00;
            font-size: 36rpx;
            margin: 0 10rpx 10rpx 0;
            text-align: center;
          }
        }
      }
    }
  }
}
.showBox {
  height: 120rpx;
}
</style>
